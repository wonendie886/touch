...
u8 TP_Init(void)
{
	...
	if(lcddev.id==0X5510||lcddev.id==0X9806)				//4.3寸电容触摸屏
	{
    GT1151_Init();			
...
//	 }	 
	...
	else if(lcddev.id==0X7789)    //3.5寸电容触摸屏
	{
	 GT1151_Init();         //判断是电容触摸屏还是电阻触摸屏
	 tp_dev.scan=GT1151_Scan;    //扫描函数指向GT1151触摸屏扫描
		tp_dev.touchtype|=0X80;     //电容屏 
		tp_dev.touchtype|=lcddev.dir&0X01;//横屏还是竖屏
		return 0;
	...
	else
	{
		...
		//PB12,13初始化设置
		GPIO_Initure.Pin=GPIO_PIN_12|GPIO_PIN_13;	//PB12/PB13 设置为上拉输入
		GPIO_Initure.Mode=GPIO_MODE_INPUT;  	  //输入模式
		GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
		GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);     //初始化
		...
		//PB14
		GPIO_Initure.Pin=GPIO_PIN_14;          	//PB14设置为推挽输出
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);     //初始化
